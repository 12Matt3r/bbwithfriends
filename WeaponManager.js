...
import { WEAPON_TYPES, SOUND_KEYS } from './Constants.js';

export const WEAPON_CONFIG = {
    [WEAPON_TYPES.ASSAULT]: {
        fireRate: 100,
        damagePerPellet: 20,
        pelletCount: 1,
        spreadAngle: 0.01,
        ...
        reloadSound: SOUND_KEYS.RELOAD_RIFLE // Assuming assault is rifle-like
    },
    [WEAPON_TYPES.SCOUT]: {
        fireRate: 150,
        damagePerPellet: 15,
        ...
        silenced: true,
        canSprintShoot: true,
        reloadSound: SOUND_KEYS.RELOAD_PISTOL // Assuming scout is pistol-like
    },
    [WEAPON_TYPES.HEAVY]: {
        fireRate: 700,
        damagePerPellet: 10,
        ...
        auto: false,
        canSprintShoot: false,
        reloadSound: SOUND_KEYS.RELOAD_SHOTGUN // Assuming heavy is shotgun-like
    }
};
...
export class WeaponManager {
    ...
    shoot(cameraWorldDirection, cameraWorldPosition) {
        ...
        // Ensure this.player.weaponType is a valid key (it should be if Player.js sets it from WEAPON_TYPES)
        const recoilConfig = (WEAPON_CONFIG[this.player.weaponType] && WEAPON_CONFIG[this.player.weaponType].recoil) || 0.01;
        this.recoilOffset.x += (Math.random() - 0.5) * recoilConfig * 2;
        this.recoilOffset.y += recoilConfig;
...
import * as THREE from 'three';
import { GLTFLoader } from 'three/addons/loaders/GLTFLoader.js';
import { WEAPON_TYPES, SOUND_KEYS, MESSAGE_TYPES } from './Constants.js'; // Added MESSAGE_TYPES
...
export class WeaponManager {
    ...
    shoot(cameraWorldDirection, cameraWorldPosition) {
        ...
        if (this.gameCore.networkManager) {
            ...
            this.gameCore.networkManager.send({
                type: MESSAGE_TYPES.PLAYER_SHOT,
        ...
        if (weaponConfig.silenced) {
            this.audioManager.playSound(SOUND_KEYS.SILENCED_SHOT, soundPosition);
        } else {
            this.audioManager.playSound(SOUND_KEYS.SHOOT, soundPosition);
        }
    ...
    reload() { // Simplified as Player.startReload handles internal state and sound
        if (this.isReloading) return false; // Already in the process (checked by player.startReload too)
        
        // Attempt to start reload process in Player.js
        const reloadStarted = this.player.startReload(); // Player.startReload() now returns boolean
        
        if (reloadStarted) {
            this.isReloading = true; // WeaponManager can track this for its own logic if needed
                                   // but Player.js is the source of truth for reload state.
            // The actual reload completion and ammo update is handled by Player.updateStateAndMovement
            // Sound is also played in Player.startReload()
            
            // Set a timeout to reset WeaponManager's local isReloading,
            // aligning with when Player's reload is expected to finish.
            // This is mostly for WeaponManager's internal state if it needs to prevent actions.
            const weaponConfig = this.player.getWeaponConfig();
            const reloadTime = weaponConfig ? weaponConfig.reloadTime : 2000; // Default if no config
            setTimeout(() => {
                this.isReloading = false;
            }, reloadTime);
